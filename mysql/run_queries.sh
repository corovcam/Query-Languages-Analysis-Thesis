...
for file in queries/testing/*.sql; do
    ...
    for i in {1..20}; do
        ...
        mysql --user=root -e "TRUNCATE performance_schema.events_statements_history_long;" ecommerce |& tee -a "$log_file"
        ...
        mysql --user=root --batch --unbuffered --skip-comments ecommerce < "$file" > /dev/null 2>> "$log_file" || \
            { echo "-1" | tee -a "$query_csv_file" && break; }
        mysql -sN --user=root --batch \
