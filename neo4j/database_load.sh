...
dump_dir="$HOME/Query-Languages-Analysis-Thesis/neo4j/dumps/data_128k" # NOTE: Change this to the directory where the dump is located
...
neo4j-admin database load neo4j --from-path=/backups --overwrite-destination || echo "[$(date +"%Y-%m-%d %T")] Database load failed" |& tee -a "$log_file"
